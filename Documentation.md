...
1. id_by_name(api_token,user_name) :- Get a user's hackthebox ID by his/her name
...
2. user_by_identifier(user_identifier) :- Get's user's full information from his identifier
...
3. get_matrix(api_token,box_name) :- Gets the matrix of a box by its name
...
4. box_info(api_token,box_name) :- Gets general information of the box
  api_token and box_name both in string format
...
5. walkthrough(box_name) :- searches ippsec.rocks for any walkthrough for the specified box name
